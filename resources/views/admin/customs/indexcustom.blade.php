...
@section('js')
    <script>
        $(document).ready(function (e){
            $('.hrefbtn').attr('href',"{{route('createcustomer')}}");
                ...
                $.ajax(
                    {
                        ...
                        success: function (data) {
                            if (data == "ok") {
                                // location.reload();
                                toastr.success('مشترک با موفقیت حذف شد.');
...
@section('content')
    ...
    <div class="container card px-5" style="margin-top: 230px;background-color: gainsboro">
        ...
        <table class="align-right table table-hover table-dark table-striped mytable w-100 pt-2">
            ...
            @foreach($personals as $key=>$personal)
                <tr data-id="{{$personal->id}}">
                    ...
                    <td>
                        <a href="{{route('reserveuser',$personal->id)}}" title="تاریخچه" class="btn btn-success"><i class="fa fa-history"></i> </a>

                        <a href="{{route('gettransactionbyuser',$personal->id)}}" title="تراکنش مالی" class="btn btn-info m-1"><i class="fa fa-money-bill"></i> </a>
        ...
        @else
            <div class="alert-danger p-1 my-1">فعلا مشترکی ثبت نشده است.</div>
        @endif
    </div>
@endsection
