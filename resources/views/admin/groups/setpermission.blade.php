...
@section('content')
            ...
            <div class="sub-header mt-3 py-2 px-3 align-self-center d-sm-flex w-100 rounded"
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0" style="font-size: 15px">
                        ...
                        <li class="breadcrumb-item" style="background: #d0e7ff"><a href="{{route('group.create')}}">ثبت سمت</a>
                        </li>
                        <li class="breadcrumb-item active" style="background: #d0e7ff" aria-current="page"> افزودن دسترسی برای سمت {{$group->name}}
                        </li>
                    </ol>
                </nav>
            </div>
    ...
    <!-- END: Breadcrumbs-->
    <!-- START: Form-->
    <form method="post" id="setGroup" action="{{route('setpermissionsgroup')}}">
        @csrf
        <input name="gId" class="d-none" value="{{$group->id}}">
