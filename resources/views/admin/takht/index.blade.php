...
@section('js')
    <script>
        ...
        $('.hrefbtn').attr('href',"{{route('takht.create')}}");
        ...
        $('.dataTables_filter').after('<div class="col-sm-12 col-md-6 mt-2 order-1"><a href="{{route('takht.create')}}" class="btn btn-success mb-2" style="width:20%">افزودن</a></div>');
            ...
            <div class="sub-header mt-3 py-2 px-3 align-self-center d-sm-flex w-100 rounded" style="border-radius: 20px!important;background: #d0e7ff;box-shadow: rgba(0, 0, 0, 0.25) 0px 5px 15px;">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0" style="font-size: 15px">
                        ...
                        <li class="breadcrumb-item active" style="background: #d0e7ff" aria-current="page">{{$room->pansion->name}} در طبقه {{$room->floor}} اتاق {{$room->roomnumber}} </li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="container card px-5" style="margin-top: 230px;background-color: gainsboro">
        @if($takhts!='notfound')
            <table class="align-right table table-hover table-dark table-striped mytable w-100 pt-2">
                ...
                @foreach($takhts as $key=>$takht)
                <tr data-id="{{$takht->id}}">
                        {{--             @if($tik->ferestande=\Illuminate\Support\Facades\Auth::id() or $tik->girande=\Illuminate\Support\Facades\Auth::id())--}}
                        <td>{{++$key}}</td>
                        <td>{{$takht->floor!=null ? $takht->floor : '-'}}</td>
                        <td>{{$takht->takhtnumber!=null ? $takht->takhtnumber : '-'}}</td>
                        <td>{{$takht->takhtnumber!=null ? $takht->status : '-'}}</td>

                        <td>
                            <button class="btn text-white btn-success py-2" style="padding: 10px 12px" title="گالری" data-id="{{$takht->id}}" data-toggle="modal" data-target="#photo-modal-{{$key}}"><i class="fa fa-photo-film"></i> </button>
                            <a href="{{route('takht.edit',$takht->id)}}" class="btn text-white btn-warning py-2" title="ویرایش" data-id="{{$takht->id}}"><i class="fas fa-edit"></i> </a>
                            <button  class="btn text-white btn-danger py-2" title="حذف" data-id="{{$takht->id}}" data-toggle="modal" data-target="#delete-modal-{{$key}}"><i class="fas fa-trash"></i> </button>
                            <!-- modal photo -->
                                            ...
                                            <div class="container-fluid">
                                                <div class="row">
                                                    <div class="col-12 text-center position-relative home-slider mb-3" style="height: 450px;width: 450px">
                                                        @if(isset($takht->photo[0]->path))
                                                            <img class="bigImg" style="width:450px!important;height: 450px!important;" src="{{asset($takht->photo[0]->path)}}" data-id="{{$takht->photo[0]->id}}">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body text-dark" >
                                            <div style="text-align: right">
                                                <p>آیا می خواهید تخت حذف شود؟</p>
                                            </div>
                                            <div style="text-align: left">
                                                <button data-dismiss="modal" class="btn text-white btn-danger delete-btn" title="حذف" data-id="{{$takht->id}}" data-toggle="modal" data-target="#delete-modal-{{$key}}">حذف </button>
        ...
        @else
            <div class="alert-danger p-1">تختی برای اتاق ثبت نشده است.</div>
        @endif
        </div>
@endsection
