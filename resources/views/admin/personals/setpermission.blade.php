...
@section('js')
    <script>
        $(document).ready(function (){
            ...
            $("#setPermission").submit(function (e){
...
@section('content')
            ...
            <div class="sub-header mt-3 py-2 px-3 align-self-center d-sm-flex w-100 rounded" style="border-radius: 20px!important;background: #d0e7ff;box-shadow: rgba(0, 0, 0, 0.25) 0px 5px 15px;">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0" style="font-size: 15px">
                        ...
                        <li class="breadcrumb-item active" style="background: #d0e7ff" aria-current="page">افزودن دسترسی برای پرسنل {{$personal->name.' '.$personal->family}}</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <!-- END: Breadcrumbs-->
    <!-- START: Form-->
    <form method="post" id="setPermission" action="{{route('setPermissions')}}">
        @csrf
        ...
        <div class="row text-center mt-5 mb-5">
          ...
          <option value="{{$p->id}}">{{$p->permissionName}}</option>
            ...
            <div class="col-xs-5 col-md-5">
                <select name='pId[]' id="multiselect_to" class="form-control" size="8" multiple="multiple">
                </select>
            </div>
            </div>
            <div class="col-md-4 row">
                                ...
                                <div class="col-12  text-center">
                                    <div class="col-12 mb-3">
                                        <button type="submit" class="w-50 btn btn-success pt-2 pb-2" id="permissionsubmit">ثبت</button>
                                    </div>
                                    <div class="col-12 mb-3">
