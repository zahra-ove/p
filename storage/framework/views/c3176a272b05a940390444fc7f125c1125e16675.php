...
<?php $__env->startSection('js'); ?>
    <script>
        ...
        $(document).ready(function (e) {
            ...
            $.ajax({
                url: '<?php echo e(asset('admin/getallordersreturn')); ?>',
                async: false,
            ...
            $.ajax({
                url: '<?php echo e(asset('admin/getorderlimitoffset')); ?>/' + '10' + '/0',
                async: false,
                success: function (data) {
   ...
   <td>${item.fullname}</td>
        <td>${item.mobilecode}</td>
                    ...
                    <td>${item.vaziat}</td>
                    <td>   <a href="<?php echo e(asset('admin/getstatusmalibyorder')); ?>/${item.id}" style="width: 40px" class="btn btn-secondary"><i class="fa fa-calculator"></i> </a></td>
                    </tr>`)
                        })
                    } else {
                        $('.notfound').remove();
            ...
            $(document).on('click', ".pagination a", function (ep) {
                ...
                if ($(this).hasClass('active') == false) {
                    if ($(this).hasClass('his') == false && $(this).hasClass('searched') == false) {
                        ...
                        $.ajax({
                            url: '<?php echo e(asset('admin/getorderlimitoffset')); ?>/' + counts + '/' + preCounts,
                            async: false,
                            success: function (data) {
                                if (data != 'notfound') {
                                    $('.pagination').empty();
   ...
   <td>${item.fullname}</td>
        <td>${item.mobilecode}</td>
                    ...
                    <td>   <a href="<?php echo e(asset('admin/getstatusmalibyorder')); ?>/${item.id}" style="width: 40px" class="btn btn-secondary"><i class="fa fa-calculator"></i> </a></td>
                        ...
                        let countDateOrders = "";
                        $.ajax({
                            url: '<?php echo e(asset('admin/getallordersbydate')); ?>/' + from + '/' + to,
                            async: false,
                            success: function (data) {
                        ...
                        $.ajax(
                            {
                                url: "<?php echo e(asset('admin/getallordersbydate')); ?>/" + from + '/' + to + '/' + counts + '/' + preCounts,
                                success: function (data) {
                                    if (data != 'notfound') {
                                        $('.pagination').empty();
                    ...
                    <td>   <a href="<?php echo e(asset('admin/getstatusmalibyorder')); ?>/${item.id}" style="width: 40px" class="btn btn-secondary"><i class="fa fa-calculator"></i> </a></td>
                        ...
                        let countDateOrders = "";
                        $.ajax({
                            url: '<?php echo e(asset('admin/getorderbysearch')); ?>/' + column + '/' + output,
                            async: false,
                            success: function (data) {
                                countDateOrders = data;
                            }
                        ...
                        $.ajax(
                            {
                                url: "<?php echo e(asset('admin/getorderbysearchlimitoffset')); ?>/" + column + '/' + column + '/' + counts + '/' + preCounts,
                                success: function (data) {
                                    if (data != 'notfound') {
                                        $('.pagination').empty();
                                        let countpages = Math.ceil(countDateOrders.length / 10);
            ...
            $('.hrefbtn').attr('href', "<?php echo e(route('pansion.create')); ?>");
            $('#searchDate').click(function (es) {
                if ($('#from').val().length != 0 && $('#to').val().length != 0) {
                    ...
                    $.ajax(
                        {
                            url: "<?php echo e(asset('admin/getallordersbydate')); ?>/" + from + '/' + to + '/' + '10' + '/' + '0',
                            success: function (data) {
            ...
            $('#searchColumn').click(function (es) {
                if ($('#column').val().length != 0 && $('#output').val().length != 0) {
                    ...
                    $.ajax(
                        {
                            url: "<?php echo e(asset('admin/getorderbysearchlimitoffset')); ?>/" + column + '/' + output + '/' + '10' + '/' + '0',
                            success: function (data) {
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="container card px-5" style="margin-top: 230px;background-color: gainsboro">
        <div class="row ">
            <div class="col-lg-6 col-12 row text-right justify-content-between ml-1">
    ...
    background-color: beige;">
                        <span class="form-label mb-2 position-relative" style="top: 24%">ستون</span>
                    </div>
                    <div class="col-9 p-0 text-left">
                        <select name="column" class="form-control select2" id="column">
                            <option value=""></option>
                            <option value="name">نام و نام خانوادگی</option>
                            <option value="ncode">کد ملی</option>
                            <option value="mobilecode">موبایل</option>
                            <option value="order_number">شماره سفارش</option>
                            <option value="pansion">خوابگاه</option>
                            <option value="karshenas">ثبت کننده</option>
                        </select>
                    </div>
                </div>
                <div class="col-lg-5 col-12 mb-3 mt-5 row">
    ...
    border-radius: 5px;
    background-color: beige;">
                        <span class="form-label mb-2 position-relative" style="top: 24%">کلمه</span>
                    </div>
                    <div class="col-9 p-0 ">
                        <input name="output" id="output" class="form-control">

                    </div>
                </div>
                <div class="col-2 mb-3 mt-5">
                    <span class="btn btn-secondary" id="searchColumn">جستجو</span>
                </div>
            </div>
                ...
                <div class="col-12 m-auto justify-content-center row">
                    <div class="pagination mb-2">
                        ...
                        <?php for($i=1;$i<=$pages;$i++): ?>
                            <?php if($i=='1'): ?>
                                <a page="<?php echo e($i); ?>" href="#" class="active"><?php echo e($i); ?></a>
                            <?php else: ?>

                                <a page="<?php echo e($i); ?>" href="#"><?php echo e($i); ?></a>
...
<?php echo $__env->make('admin.master.home', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\pansion3\resources\views/admin/orders/allorders.blade.php ENDPATH**/ ?>